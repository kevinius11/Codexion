...
t_waiter create_ticket(t_coders *coder)
{
	t_waiter ticket;
	ticket.coder_id = coder->id;

	if (coder->data->scheduler == FIFO)
		ticket.timestamp = get_time_ms();
	else
		ticket.timestamp = coder->last_compilation + coder->data->time_to_burnout;
...
void	take_dongle(t_coders *coder, t_dongle *dongle)
{
	...
	ticket = create_ticket(coder);

	pthread_mutex_lock(&dongle->mutex);

	heap_insert(&dongle->queue, ticket);

	while (!dongle->available || heap_peek(&dongle->queue).coder_id != coder->id)
